...
export default function PickCard({ pick, index, showResultButtons = false, onEdit, onDelete }) {
  ...
  // Get card border color based on result (for MyBets)
  ...
  return (
    <motion.div
      ...
      className="group"
    >
      <div className={`glass-strong rounded-2xl p-6 transition-all duration-300 relative overflow-hidden ${getCardBorderClass()}`}>
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-start justify-between mb-6">
            <div className="flex items-center gap-4">
              <motion.div
                ...
                className="w-14 h-14 rounded-xl bg-gold-gradient flex items-center justify-center text-2xl font-black text-black gold-glow"
              >
                #{pick.rank}
              </motion.div>
              <div>
            ...
            <div className="text-right">
              <div className="flex items-center gap-2 justify-end mb-1">
                ...
                <span className="text-gray-300 font-semibold">u/{pick.poster}</span>
              </div>
              <div className="text-sm text-gold-400 font-mono">{pick.poster_record}</div>
              <div className="text-xs text-gray-500">{pick.poster_win_rate}</div>
          ...
          <div className="mb-4">
            <div className="flex items-center gap-3 mb-3 flex-wrap">
              ...
              {pick.risk_level && (
                <div className={`flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-bold border ${getRiskColor(pick.risk_level)} uppercase`}>
          ...
          {/* The Pick */}
          <div className="mb-4 p-4 rounded-xl glass border border-gold-500/20 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-gold-500/5 rounded-full blur-2xl" />
            <div className="relative">
              <div className="text-xs text-gray-500 mb-1 uppercase tracking-wider font-semibold flex items-center gap-2">
          ...
          {/* Gamblina's Analysis - COLLAPSIBLE */}
          {pick.reasoning && (
            <div className="mb-4">
              <button
                onClick={() => setShowAnalysis(!showAnalysis)}
                className="w-full text-left p-4 rounded-xl glass border border-purple-500/20 hover:border-purple-500/40 transition-all relative overflow-hidden"
              >
                ...
                <div className="relative flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="ml-2 px-2 py-0.5 rounded bg-purple-500/20 text-[10px] font-bold text-purple-300">
                        AI POWERED
                      </span>
                    </div>
                  </div>
              ...
              <AnimatePresence>
                {showAnalysis && (
                  ...
                  >
                    <div className="mt-2 p-5 rounded-xl glass border border-purple-500/20 bg-gradient-to-br from-purple-500/5 to-transparent">
                      <div className="mb-4">
                        <div className="flex items-center gap-2 mb-2">
                          <div className="h-px flex-1 bg-purple-500/20"></div>
                          <span className="text-xs text-purple-400 uppercase tracking-wider font-semibold">Why {pick.confidence}% Confident</span>
                          <div className="h-px flex-1 bg-purple-500/20"></div>
                        </div>
                      ...
                      {keyFactors && keyFactors.length > 0 && (
                        <div className="space-y-2">
                          <div className="text-xs text-purple-400 uppercase tracking-wider font-semibold flex items-center gap-2">
          ...
          {pick.original_comment && (
            <div>
              <button
                onClick={() => setShowOriginal(!showOriginal)}
                className="w-full text-left p-4 rounded-xl glass border border-blue-500/20 hover:border-blue-500/40 transition-all"
              >
              ...
              <AnimatePresence>
                {showOriginal && (
                  <motion.div
                    ...
                    className="overflow-hidden"
                  >
                    <div className="mt-2 p-4 rounded-xl glass border border-blue-500/20 bg-blue-500/5">
      ...
      <div className="space-y-2">
        {/* Admin Controls */}
        {user?.role === 'admin' && (
          <div className="flex gap-2 p-3 border-t border-gray-800/50 bg-gray-900/30">
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={() => onEdit && onEdit(pick)}
              disabled={deleteLoading}
              className="flex-1 py-2 px-3 rounded-lg font-bold bg-blue-600/20 text-blue-400 border border-blue-500/30 hover:bg-blue-600/30 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <motion.button
              ...
              disabled={deleteLoading}
              className="flex-1 py-2 px-3 rounded-lg font-bold bg-red-600/20 text-red-400 border border-red-500/30 hover:bg-red-600/30 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {deleteLoading ? (
        ...
        {/* User Action Buttons */}
        <div className="flex gap-2 p-4 border-t border-gray-800/50">
        {showResultButtons ? (
          // RESULT BUTTONS (for MyBets - only Won/Lost, no Push)
          <>
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
          ...
          // ACTION BUTTONS (for Live Picks - hit/track/fade)
          <>
        <motion.button
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
